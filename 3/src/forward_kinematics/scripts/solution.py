...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        numjv = len(joint_values.name)
        numlinks = len(link_names)
        # print("number of link names is {}").format(len(link_names))
        # print("number of joints is {}").format(len(joints))
        # print(numjv)
        jvdict = dict()

        for idx in range(0,numjv):
            currjointname = joint_values.name[idx]
            currjointpos = joint_values.position[idx]
            jvdict.update({currjointname:currjointpos})

        for idx in range(0,numlinks):
            currlinkname = link_names[idx]
            currjoint = joints[idx]

            ftrans = tf.transformations.translation_matrix(currjoint.origin.xyz)
            frot = tf.transformations.euler_matrix(currjoint.origin.rpy[0], 
            currjoint.origin.rpy[1], currjoint.origin.rpy[2])

            T = tf.transformations.concatenate_matrices(T, ftrans, frot)

            # tmsg = convert_to_message(T, currlinkname, "world_link")
            # all_transforms.transforms.append(tmsg)

            if currjoint.type == 'revolute':
                angle = jvdict[currjoint.name]
                axis = currjoint.axis
                qrot = tf.transformations.quaternion_about_axis(angle, axis)
                revT = tf.transformations.quaternion_matrix(qrot)
